      ...
      <ul className="space-y-4">
        {
          leaderboard.map((entry, index) => (
            <li key={index+1} className="flex flex-col md:flex-row items-start gap-4">
              <span className="bg-black p-[6px] w-[48px] h-[48px] text-3xl rounded-full text-center align-middle font-bold">{index+1}</span>
              <div className="px-10 py-6 w-full md:w-[488px] lg:w-[688px] rounded-sm border border-white/50 bg-card text-black font-medium space-y-6">
                <p>{entry.clue_text}</p>
              ...
              <p className="grid place-content-center self-center font-medium max-w-[88px] text-center text-lg">
